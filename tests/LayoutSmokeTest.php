...
foreach (['<main','id="adu-calendar-view"','<form id="adu-own-form"','aria-live="polite"','<caption>', "\\OCP\\Util::addScript('orgsuite', 'suite-navigation')", "\\OCP\\Util::addScript('adurlaub', 'components/vacation-plan')", "\\OCP\\Util::addScript('adurlaub', 'modules/vacation-app')", "\\OCP\\Util::addStyle('orgsuite', 'suite-navigation')", 'data-orgsuite data-suite="ad" data-current-app="adurlaub"'] as $contract) if (!str_contains($template, $contract)) throw new RuntimeException("UI-Vertrag fehlt: {$contract}");
