...
foreach (['<main','role="tablist"','id="adu-tab-settings"','aria-controls="adu-settings-view"','id="adu-settings-view"','aria-labelledby="adu-tab-settings"','>Einstellungen</button>','<form id="adu-own-form"','aria-live="polite"','<caption>', "script('orgsuite', 'suite-navigation')", "style('orgsuite', 'suite-navigation')", 'data-orgsuite data-suite="ad" data-current-app="adurlaub"'] as $contract) if (!str_contains($template, $contract)) throw new RuntimeException("UI-Vertrag fehlt: {$contract}");
