...
final class ApiController extends Controller {
    public function __construct(IRequest $request, private VacationAccessService $access, private VacationService $vacations, private VacationTeamService $teams, private LoggerInterface $logger) { parent::__construct(Application::APP_ID, $request); }
    ...
    #[NoAdminRequired]
    public function setDayStatus(string $teamId, int $year, string $employeeUid, string $date, string $status): JSONResponse { $team = $this->teams->get($teamId); if ($team === null || !$team->contains($employeeUid) || !preg_match('/^' . preg_quote((string)$year, '/') . '-\d{2}-\d{2}$/', $date)) return $this->denied(); $existing = $this->vacations->existingCovering($employeeUid,$date); if ($existing !== null && !$this->access->canManageStatus($employeeUid,$existing->status())) return $this->denied(); if (!$this->access->canManageStatus($employeeUid,$status)) return $this->denied(); try { return new JSONResponse(['id'=>$this->vacations->setStatusForDate($employeeUid,$date,$status,$this->access->currentUser()?->getUID() ?? '')]); } catch (VacationConflictException $error) { return new JSONResponse(['error'=>$error->getMessage(),'conflicts'=>$error->conflicts()],Http::STATUS_CONFLICT); } catch (VacationOverlapException $error) { return new JSONResponse(['error'=>$error->getMessage()],Http::STATUS_CONFLICT); } catch (\Throwable) { return new JSONResponse(['error'=>'Der Urlaubsstatus konnte nicht gespeichert werden.'],Http::STATUS_BAD_REQUEST); } }
    private function save(?int $id, array $payload): JSONResponse { if (!$this->access->isVisibleEmployee($payload['employeeUid']) || !$this->access->canManageStatus($payload['employeeUid'], $payload['status'])) return $this->denied(); try { return new JSONResponse(['id' => $this->vacations->save($payload, $id, $this->access->currentUser()?->getUID() ?? '')]); } catch (VacationConflictException $error) { return new JSONResponse(['error'=>$error->getMessage(),'conflicts'=>$error->conflicts()],Http::STATUS_CONFLICT); } catch (VacationOverlapException $error) { return new JSONResponse(['error'=>$error->getMessage()],Http::STATUS_CONFLICT); } catch (\Throwable) { return new JSONResponse(['error' => 'Der Urlaub ist ungültig.'], Http::STATUS_BAD_REQUEST); } }
