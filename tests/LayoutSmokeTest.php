...
foreach (['<main','id="adu-calendar-view"','<form id="adu-own-form"','aria-live="polite"','<caption>', "script('orgsuite', 'suite-navigation')", "script('adurlaub', 'components/vacation-plan')", "script('adurlaub', 'modules/vacation-app')", "style('orgsuite', 'suite-navigation')", 'data-orgsuite data-suite="ad" data-current-app="adurlaub"'] as $contract) if (!str_contains($template, $contract)) throw new RuntimeException("UI-Vertrag fehlt: {$contract}");
