(function () {
    ...
    const ids = ['team','year','calendar-head','calendar-body','own-form','own-requests','conflicts','calendar-view'];
    ...
    elements['own-form'].addEventListener('submit',async event=>{ event.preventDefault(); const formElement=event.currentTarget; const form=new FormData(formElement); try { await save({employeeUid:state.currentUser.uid,startDate:form.get('startDate'),endDate:form.get('endDate'),status:'planned',note:form.get('note')}); formElement.reset(); await loadYear(); notice.success('Urlaub eingetragen.'); } catch(error){ notice.error(error); } });
