(function () {
    ...
    async function loadSettings() { try { const settings=await client.request('/api/settings'); elements['tab-settings'].hidden=false; elements['peer-settings'].replaceChildren(...Object.entries(settings.peerApproval).map(([group,enabled])=>{ const label=node('label'); const input=document.createElement('input'); input.type='checkbox'; input.name=group; input.checked=enabled; label.append(input,document.createTextNode(` ${group==='ad-ASN-*'?'ASN-Teamkolleg*innen':group}`)); return label; })); } catch (_) { showView('calendar'); } }
